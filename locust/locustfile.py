        ...
        with self.db_connection.cursor() as cursor:
                ...
                if MyUser.user_count == MyUser.total_users:
                    for request_name, data in MyUser.aggregated_data.items():
                        ...
                        query = sql.SQL("""
                            INSERT INTO locustData (url, timestamp, request_count, failure_count, avg_response_time, min_response_time, max_response_time, median_response_time)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        """)

                        cursor.execute(query, (url, self.get_current_time(), request_count, failure_count, avg_response_time, min_response_time, max_response_time, median_response_time))
